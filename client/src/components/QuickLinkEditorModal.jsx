...
const QuickLinkEditorModal = ({ open, onClose, item, index, onSave, onDelete }) => {
  ...
  const [form, setForm] = useState({ en: { title: '', desc: '' }, ta: { title: '', desc: '' }, common: { to: '' } });
  ...
  useEffect(() => {
    ...
    const to = v.to || '';
    setForm({ en: { title: title.en || '', desc: desc.en || '' }, ta: { title: title.ta || '', desc: desc.ta || '' }, common: { to } });
  }, [item]);
    ...
    <div className="fixed inset-0 z-60 flex items-start justify-center p-6 overflow-auto">
      ...
      <div className="relative max-w-xl w-full bg-white rounded shadow-xl p-6 z-70 max-h-[calc(100vh-48px)] overflow-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold">{index != null ? t('editor.edit_link', { lng: currentLang }) : t('editor.add_link', { lng: currentLang })}</h3>
        ...
        <div className="grid grid-cols-1 gap-3 mb-4">
          ...
          <input value={form[lang].desc} onChange={e => setForm(f => ({ ...f, [lang]: { ...f[lang], desc: e.target.value } }))} className="border px-3 py-2 rounded" />

          <label className="text-sm font-semibold">{t('editor.url_internal', { lng: lang })}</label>
          <input value={form.common.to} onChange={e => setForm(f => ({ ...f, common: { ...f.common, to: e.target.value } }))} className="border px-3 py-2 rounded" />
        </div>

        <div className="flex gap-3">
          <button onClick={() => onSave({ title: { en: form.en.title, ta: form.ta.title }, desc: { en: form.en.desc, ta: form.ta.desc }, to: form.common.to }, index)} className="px-4 py-2 bg-[#d21d1d] text-white rounded">{t('editor.save', { lng: currentLang })}</button>
          <button onClick={() => setForm({ en: { title: '', desc: '' }, ta: { title: '', desc: '' }, common: { to: '' } })} className="px-4 py-2 border rounded">{t('editor.clear', { lng: currentLang })}</button>
...
export default QuickLinkEditorModal;
